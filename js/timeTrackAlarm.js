...
const restartSVG =
  ...
  playSVG =
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="header__svg" viewBox="0 0 16 16"><path d="m11.596 8.697-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393z"/></svg>',
  pauseSVG =
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="header__svg" viewBox="0 0 16 16"><path d="M5.5 3.5A1.5 1.5 0 0 1 7 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5zm5 0A1.5 1.5 0 0 1 12 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5z"/></svg>',
  stopAlarmSVG =
...
const setAlarm = () => {
  ...
  document.getElementById("playAlarm").innerHTML = playSVG;
  document.getElementById("alarmTimer").innerHTML = `${String(hours).padStart(
    2,
    "0"
  )}:${String(min).padStart(2, "0")}:${String(sec).padStart(2, "0")}`;
};

const limitTwoIntegers = (input) => {
...
function playPauseAlarm() {
  if (!isAlarmInit) {
    if (!sound.paused) {
      ...
      if (!alarmInDate)
        alarmInDate = new Date(new Date().getTime() + alarmInMiliSec);
      ...
      idInterval = setInterval(() => {
        ...
        if (restingTime <= 0) {
          ...
          document.getElementById("alarmTimer").innerHTML =
            MiliSecToHourMinSec(restingTime);
        }
      }, 500);
...
const MiliSecToHourMinSec = (TotalMiliSec) => {
  let hour = String(Math.floor(TotalMiliSec / (1000 * 60 * 60))).padStart(
    2,
    "0"
